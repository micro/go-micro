...
1. **Fix atlascloud plan-delegate harness missing delegated notify side effect** ([#3760](https://github.com/micro/go-micro/issues/3760)) — PR #3783 closed the buyer-recipient universe blocker (#3772), leaving plan/delegate as the live atlascloud/minimax 0→hero services → agents → workflows failure. Keep this ahead of broader hardening until the conductor → delegated comms → notify path reliably proves the multi-agent side effect developers see in the harness.
