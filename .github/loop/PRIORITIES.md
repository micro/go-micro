...
1. **Require real notification side effects in AtlasCloud plan/delegate conformance** ([#3711](https://github.com/micro/go-micro/issues/3711)) — PR #3713 shipped the no-secret first-agent transcript, so the adoption on-ramp item is done and drops out of the queue. The highest-value Now-phase gap is now live-provider correctness: AtlasCloud/Minimax can emit an unavailable provider-specific tool call while the conductor marks delegation complete, which breaks the services → agents → workflows promise more directly than parser polish. Make the harness fail/retry until the registered delegate/comms notification path actually executes, with a CI-safe regression for unknown tool calls.
2. **Expose `fallback_echo` during A2A streaming fallback conformance** ([#3560](https://github.com/micro/go-micro/issues/3560)) — this remains the next Now-phase interop/conformance gap: it protects the A2A streaming promise developers see in the README and site by ensuring the non-native streaming fallback path receives the tool surface, while staying scoped and CI-verifiable.
