...
1. **Make AtlasCloud universe concierge notification deterministic** ([#3718](https://github.com/micro/go-micro/issues/3718)) — PR #3720 closed the plan/delegate unknown-tool recovery gap, so the top remaining Now-phase risk is the adjacent live-provider universe regression: checkout resume succeeds and the agent/tool wrapper observes activity, but the buyer notification side effect is missing or non-deterministic. Keep this above parser polish because it directly tests the services → agents → workflows lifecycle under durable flow resume, A2A reachability, and real provider tool execution.
