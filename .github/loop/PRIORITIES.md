...
1. **Fix atlascloud universe missing concierge notify side effect** ([#3804](https://github.com/micro/go-micro/issues/3804)) — the first-agent/0→hero docs wayfinding has shipped, so the highest-value blocker returns to the live harness: `atlascloud / universe` resumed the checkout flow but completed without the required buyer notification. This is directly on the services → agents → workflows contract and must stay a real side-effect assertion, not a weakened text-only pass.
2. **Fix atlascloud plan-delegate missing delegated notify side effect** ([#3805](https://github.com/micro/go-micro/issues/3805)) — keep the delegated-plan side-effect gap adjacent to #3804 because it is the same adoption-critical harness seam: agents can create the expected work but still miss the observable notify side effect. Recent plan-delegate recovery work helps, but this open live-provider failure needs a focused follow-up run/fix before deeper feature work.
