...
1. **Add a no-secret first-agent quickstart transcript** ([#3708](https://github.com/micro/go-micro/issues/3708)) — PR #3706 closed the delegated-plan correctness blocker, leaving no open codex PRs in flight. The current adoption steer says the next highest-value gap should be the lowest-friction first agent success: README and docs now point to first-agent/debug/0→hero material, but the first useful agent transcript still asks new developers to bridge from the no-secret support example to the live-key walkthrough themselves. A CI-backed, no-secret transcript tightens the install → scaffold/run → chat/inspect on-ramp without changing public APIs.
2. **Expose `fallback_echo` during A2A streaming fallback conformance** ([#3560](https://github.com/micro/go-micro/issues/3560)) — this remains the top Now-phase interop/conformance gap after the adoption on-ramp: it protects the A2A streaming promise developers see in the README and site by ensuring the non-native streaming fallback path receives the tool surface, while staying scoped and CI-verifiable.
