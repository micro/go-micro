...
1. **Make provider-conformance plan/delegate complete deterministically** ([#3724](https://github.com/micro/go-micro/issues/3724)) — PR #3726 closed the universe concierge notification gap, leaving the freshest live-provider regression in the same Now-phase seam: services create the expected tasks and the delegated agent calls `notify`, but the durable flow times out instead of returning a single successful terminal result. Keep this first because it is the current end-to-end adoption contract for services → agents → workflows under real provider tool execution, delegation, and side-effect accounting.
