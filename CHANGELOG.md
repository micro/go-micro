...
### Added
- **Scoped store state** — `store.Scope(s, database, table)` returns a store handle that confines every operation to a database/table without mutating the shared store (unlike `Init(Table(...))`, which is process-global and races between co-located components). Agents now keep memory and plan in their own table (`agent/{name}`), and flow runs in theirs (`flow/{name}`), instead of a single global table. Services already scope by name.
