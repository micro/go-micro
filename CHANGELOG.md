...
### Added
- **A2A protocol gateway** — `gateway/a2a` exposes registered agents over the open Agent2Agent (A2A) protocol so agents on other frameworks can discover and call them. Agent Cards are generated from registry metadata (the same way the MCP gateway derives tools), and incoming A2A tasks are translated to the agent's existing `Agent.Chat` RPC; no per-agent code. v1 covers the synchronous JSON-RPC binding (`message/send` → completed `Task`, `tasks/get`, card discovery); streaming and push notifications are advertised as unsupported. Run it with `micro a2a serve`. (`gateway/a2a/`, `cmd/micro/a2a/`)
