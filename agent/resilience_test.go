...
func TestAskRetriesTransientErrorsThenSurfacesStructuredError(t *testing.T) {
	...
	if !errors.As(err, &retryErr) {
		t.Fatalf("Ask error = %T %v, want *model.RetryError", err, err)
	}
...
func TestModelRetryDoesNotDuplicateCheckpointedToolSideEffects(t *testing.T) {
	...
	fakeGen = func(ctx context.Context, opts model.Options, req *model.Request) (*model.Response, error) {
		...
		res := opts.ToolHandler(ctx, model.ToolCall{ID: "create-1", Name: "external.create", Input: map[string]any{"title": "Retry safe"}})
		...
		return &model.Response{Reply: "done", ToolCalls: []model.ToolCall{{ID: "create-1", Name: "external.create", Input: map[string]any{"title": "Retry safe"}, Result: res.Content}}}, nil
	}
	defer func() { fakeGen = nil }()
...
func TestAskRateLimitFailureSuggestsPreflightAndInspect(t *testing.T) {
	...
	if model.ClassifyError(err) != model.ErrorKindRateLimited {
		t.Fatalf("ClassifyError(wrapped error) = %q, want rate_limited", model.ClassifyError(err))
	}
}
...
func TestCanceledAskContextSkipsToolExecution(t *testing.T) {
	fakeGen = func(ctx context.Context, opts model.Options, req *model.Request) (*model.Response, error) {
		...
		cancel()
		res := opts.ToolHandler(canceled, model.ToolCall{ID: "call-1", Name: toolPlan, Input: map[string]any{
...
func TestAskCancellationDuringToolCallFailsRun(t *testing.T) {
	fakeGen = func(ctx context.Context, opts model.Options, req *model.Request) (*model.Response, error) {
		...
		res := opts.ToolHandler(ctx, model.ToolCall{ID: "call-1", Name: "cancel-self"})
		if !strings.Contains(res.Content, context.Canceled.Error()) {
			t.Fatalf("tool result = %q, want cancellation error", res.Content)
		}
		return &model.Response{Reply: "should not succeed"}, nil
	}
	defer func() { fakeGen = nil }()
...
func TestSlowProviderTimeoutPreventsLateToolSideEffects(t *testing.T) {
	...
	fakeGen = func(ctx context.Context, opts model.Options, req *model.Request) (*model.Response, error) {
		...
		res := opts.ToolHandler(ctx, model.ToolCall{ID: "late-1", Name: "external.create", Input: map[string]any{"title": "too late"}})
		...
		return &model.Response{Reply: "late", ToolCalls: []model.ToolCall{{ID: "late-1", Name: "external.create", Input: map[string]any{"title": "too late"}, Result: res.Content}}}, nil
	}
	defer func() { fakeGen = nil }()
