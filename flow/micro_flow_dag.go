...
func (fl *microFlow) flowHandler(req interface{}) {
		...
		for _, req := range vs.Requires {
			if req == "all" {
				for _, ve := range steps {
					if ve.String() != vs.String() && !include(ve.Requires, "all") {
						g.AddEdge(ve, vs)
					}
				}
				break requiresLoop
	...
	requiredLoop:
		for _, req := range vs.Required {
			if req == "all" {
				for _, ve := range steps {
					if ve.String() != vs.String() && !include(ve.Required, "all") {
						g.AddEdge(vs, ve)
					}
				}
				break requiredLoop
	...
	*/
	...
	var gsteps []dag.Vertex
	gsteps, err = g.GetOrderedDescendants(root)
	if err != nil {
		return
	}

	gsteps = append([]dag.Vertex{root}, gsteps...)

	for _, gstep := range gsteps {
		step := gstep.(*Step)
		log.Printf("step %s\n", step.String())
