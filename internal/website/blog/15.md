...
Agents don't call each other's services directly. They communicate through the system — the same registry, the same broker that services already use.

When the project management agent needs to notify someone, it doesn't import the notification service or call its endpoints. It sends a message: "notify Alice that her tasks were rescheduled." The communication agent receives it and handles the details.
