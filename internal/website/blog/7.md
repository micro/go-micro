...
```json
{
  "tools": [
    {
      "name": "platform.UserService.Signup",
    ...
    {
      "name": "platform.PostService.Create",
...
```json
→ platform.PostService.TagPost {"post_id": "post-2", "tag": "golang"}
→ platform.PostService.TagPost {"post_id": "post-2", "tag": "concurrency"}
```

**Step 4: Notify alice**
```json
→ platform.MailService.Send {
    "from": "carol",
    "to": "alice",
