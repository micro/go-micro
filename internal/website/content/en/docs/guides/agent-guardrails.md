...
Use `agent.WithApproval` when a decision arrives on a later request. Unlike the
legacy synchronous `ApproveTool` callback, it receives the execution context
(including caller metadata and `model.RunInfo`) and the full proposed tool call:

```go
agent.WithApproval(func(ctx context.Context, call model.ToolCall) (agent.ApprovalDecision, error) {
    return agent.ApprovalDecision{
        Status: agent.ApprovalPending,
        ID:     approvalID, // unique within this run; generated by your application
        Reason: "Review the exact tool arguments before publishing",
    }, nil
})
```

Configure `agent.WithCheckpoint` with persistent storage. A pending decision
saves the call ID, tool name, and exact inputs before returning a `PausedError`
with `RunID` and `ApprovalID`. Later tool actions in that turn are refused.
Ordinary `Resume`, `ResumePending`, and `ResumeStreamAsk` cannot bypass the
pending decision. Callback errors and missing checkpoint/approval IDs fail closed.
`ApprovalApproved` permits an immediate call; `ApprovalDenied` refuses it without
creating a pending human decision. The legacy hook remains available.

After your application authorizes the reviewer, resolve the recorded decision:

```go
response, err := agent.ResumeApproval(ctx, assistant, runID, approvalID, true, "reviewed")
```

Pass `false` to deny. The decision is saved before execution. An approved call
executes from the stored arguments before the model continues, and its result is
included in the continuation history. A denied call supplies a refusal outcome
without executing. The model does not need to regenerate the approved action.
The tool must still be available under the agent's current service/tool scope,
and execution guardrails still apply. Root-package helpers are
`micro.AgentWithApproval` and `micro.AgentResumeApproval`.

If continuation fails after the decision was saved, use ordinary `Resume` for
the run; submitting the same decision again reports that it is already resolved.
Completed, checkpointed tool results are reused after a restart. Checkpointing
still cannot guarantee exactly-once external effects if a process dies after an
action succeeds but before its result is saved. Use idempotency at the tool and
serialize execution of a run across replicas; the checkpoint backend supplies
no distributed lease. Authenticate and authorize approval endpoints in your
application, and protect stored tool inputs as application data.
