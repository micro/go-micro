...
func TestProvider_StreamWithToolsFallsBack(t *testing.T) {
	p := NewProvider(model.WithAPIKey("test-key"))
	_, err := p.Stream(context.Background(), &model.Request{
		Prompt: "call a tool",
		Tools: []model.Tool{{
			Name:        "fallback_echo",
...
func TestProvider_GenerateToolCallEmptyFollowUpUsesToolResult(t *testing.T) {
	...
	p := NewProvider(
		model.WithAPIKey("test-key"),
		model.WithBaseURL(ts.URL),
		model.WithToolHandler(func(ctx context.Context, call model.ToolCall) model.ToolResult {
			if call.Name != "conformance_echo" {
				t.Fatalf("tool name = %q, want conformance_echo", call.Name)
			}
			return model.ToolResult{ID: call.ID, Content: `{"marker":"agent-conformance-ok"}`}
		}),
	)
...
func TestProvider_GenerateMinimaxToolRequests(t *testing.T) {
	...
	p := NewProvider(
		model.WithAPIKey("test-key"),
		model.WithBaseURL(ts.URL),
		model.WithModel("minimaxai/minimax-m3"),
		model.WithToolHandler(func(ctx context.Context, call model.ToolCall) model.ToolResult {
			return model.ToolResult{ID: call.ID, Content: `{"marker":"agent-conformance-ok"}`}
		}),
	)
...
func TestProvider_GenerateNormalizesBuiltInToolSchemas(t *testing.T) {
	...
	_, err := p.Generate(context.Background(), &model.Request{
...
func TestProvider_GenerateExecutesFollowUpToolCall(t *testing.T) {
	...
	p := NewProvider(
		...
		model.WithToolHandler(func(ctx context.Context, call model.ToolCall) model.ToolResult {
			...
			case "delegate":
				sawDelegate = true
				return model.ToolResult{ID: call.ID, Refused: model.RefusedApproval, Content: "blocked by policy"}
			default:
...
func TestProvider_GeneratePreservesFollowUpTextToolCallInReply(t *testing.T) {
	...
	p := NewProvider(
		model.WithAPIKey("test-key"),
		model.WithBaseURL("https://atlascloud.test"),
		model.WithToolHandler(func(ctx context.Context, call model.ToolCall) model.ToolResult {
			if call.Name != "conformance_echo" {
...
func TestProvider_GenerateFollowUpTextToolCallWithoutToolResultLeavesAnswerEmpty(t *testing.T) {
	...
	p := NewProvider(
		...
		model.WithBaseURL("https://atlascloud.test"),
		model.WithToolHandler(func(ctx context.Context, call model.ToolCall) model.ToolResult {
			return model.ToolResult{ID: call.ID}
		}),
	)
	resp, err := p.Generate(context.Background(), &model.Request{
		Prompt: "run conformance",
...
func TestProvider_GenerateFallsBackAfterRepeatedPartialPlanTextToolCall(t *testing.T) {
	...
	resp, err := p.Generate(context.Background(), &model.Request{
		Prompt: "plan and delegate",
		Tools:  []model.Tool{{Name: "plan", Description: "record a plan"}},
	})
	if err != nil {
		t.Fatalf("Generate returned error: %v", err)
	}
...
func TestProvider_GenerateFallsBackAfterRepeatedPartialDelegateTextToolCall(t *testing.T) {
	...
	resp, err := p.Generate(context.Background(), &model.Request{
		...
		Tools:        []model.Tool{{Name: "delegate", Description: "delegate work"}},
	})
	if err != nil {
		t.Fatalf("Generate returned error: %v", err)
	}
...
func TestProvider_GenerateRetriesMinimaxBuiltInsAsTextTools(t *testing.T) {
	...
	p := NewProvider(model.WithAPIKey("test-key"), model.WithBaseURL(ts.URL), model.WithModel("minimaxai/minimax-m3"))
	resp, err := p.Generate(context.Background(), &model.Request{
		Prompt: "plan and delegate",
		Tools: []model.Tool{
			{Name: "task_TaskService_Add", Description: "add task", Properties: map[string]any{"title": map[string]any{"type": "string"}}},
...
func TestProvider_GenerateFollowUpRetriesWithoutToolsOnBadRequest(t *testing.T) {
	...
	resp, err := p.Generate(context.Background(), &model.Request{
		Prompt: "call a tool",
		Tools:  []model.Tool{{Name: "conformance_echo", Description: "echo", Properties: map[string]any{"value": map[string]any{"type": "string"}}}},
	})
	if err != nil {
		t.Fatalf("Generate returned error: %v", err)
	}
...
func TestProvider_GenerateToolCallHTTPErrorIncludesRequestContext(t *testing.T) {
	...
	p := NewProvider(
		model.WithAPIKey("test-key"),
		model.WithBaseURL(ts.URL),
		model.WithModel("deepseek-ai/DeepSeek-V3-0324"),
	)
	_, err := p.Generate(context.Background(), &model.Request{
		Prompt: "call a tool",
		Tools: []model.Tool{{
...
func TestProvider_Registration(t *testing.T) {
	m := model.New("atlascloud", model.WithAPIKey("test"))
	if m == nil {
		t.Fatal("model.New('atlascloud') returned nil — provider not registered")
	}
...
func TestProvider_ImageRegistration(t *testing.T) {
	ig := model.NewImage("atlascloud", model.WithAPIKey("test"))
	if ig == nil {
		t.Fatal("model.NewImage('atlascloud') returned nil — image provider not registered")
	}
...
func TestProvider_GenerateImage_NoAPIKey(t *testing.T) {
	p := NewProvider()
	_, err := p.GenerateImage(context.Background(), &model.ImageRequest{Prompt: "a cat"})
	if err == nil {
		t.Error("Expected error when API key is missing, got nil")
	}
}
...
func TestProvider_ImplementsImageModel(t *testing.T) {
	var _ model.ImageModel = (*Provider)(nil)
}

func TestProvider_VideoRegistration(t *testing.T) {
	vg := model.NewVideo("atlascloud", model.WithAPIKey("test"))
	if vg == nil {
		t.Fatal("model.NewVideo('atlascloud') returned nil — video provider not registered")
	}
...
func TestProvider_GenerateVideo_NoAPIKey(t *testing.T) {
	p := NewProvider()
	_, err := p.GenerateVideo(context.Background(), &model.VideoRequest{Prompt: "a cat"})
	if err == nil {
		t.Error("Expected error when API key is missing, got nil")
	}
}
...
func TestProvider_ImplementsVideoModel(t *testing.T) {
	var _ model.VideoModel = (*Provider)(nil)
}
