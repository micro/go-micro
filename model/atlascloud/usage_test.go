...
//
// model.Response has carried a Usage field from the start, and only the streaming
...
func TestGenerateReportsUsage(t *testing.T) {
	...
	resp, err := p.Generate(context.Background(), &model.Request{Prompt: "Hello"})
	if err != nil {
	...
	want := model.Usage{InputTokens: 7, OutputTokens: 2, TotalTokens: 9}
	if resp.Usage != want {
