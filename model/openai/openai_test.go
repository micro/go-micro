...
func TestProvider_GenerateReasoningEffort(t *testing.T) {
	...
	p := NewProvider(model.WithAPIKey("test-key"), model.WithBaseURL(ts.URL), model.WithEffort("high"))
	if _, err := p.Generate(context.Background(), &model.Request{Prompt: "Hello"}); err != nil {
...
func TestProvider_ImageRegistration(t *testing.T) {
	ig := model.NewImage("openai", model.WithAPIKey("test"))
	if ig == nil {
		t.Fatal("model.NewImage('openai') returned nil — image provider not registered")
	}
	if ig.String() != "openai" {
