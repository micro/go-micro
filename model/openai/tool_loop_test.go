...
func TestGenerateToolLoopKeepsOfferingTools(t *testing.T) {
	...
	p := &Provider{}
	if err := p.Init(
		model.WithAPIKey("test"),
		model.WithBaseURL(srv.URL),
		model.WithModel("test-model"),
		model.WithToolHandler(func(ctx context.Context, tc model.ToolCall) model.ToolResult {
			toolRuns++
			return model.ToolResult{ID: tc.ID, Content: "done"}
		}),
	); err != nil {
		t.Fatalf("Init: %v", err)
	}
	...
	resp, err := p.Generate(context.Background(), &model.Request{
		...
		Tools:  []model.Tool{{Name: "step", Description: "one step", Properties: map[string]any{}}},
	})
	if err != nil {
...
func TestGenerateToolLoopIsBounded(t *testing.T) {
	...
	if _, err := p.Generate(context.Background(), &model.Request{
		Prompt: "never finish",
		Tools:  []model.Tool{{Name: "step", Description: "one step", Properties: map[string]any{}}},
	}); err != nil {
		t.Fatalf("Generate: %v", err)
