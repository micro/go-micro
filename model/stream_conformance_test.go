...
func TestStreamProvidersConformToOpenAICompatibleSSE(t *testing.T) {
	...
	for _, provider := range providers {
		...
		t.Run(provider, func(t *testing.T) {
			...
			m := model.New(provider, model.WithAPIKey("test-key"), model.WithBaseURL(ts.URL))
			if m == nil {
				t.Fatalf("model.New(%q) returned nil", provider)
			}
			stream, err := m.Stream(context.Background(), &model.Request{
				SystemPrompt: "system",
			...
			if usage.Reply != "" || usage.Usage != (model.Usage{InputTokens: 3, OutputTokens: 2, TotalTokens: 5}) {
...
func TestStreamProvidersCloseCancelsInFlightRequest(t *testing.T) {
	for _, provider := range conformingStreamProviders(t) {
		...
		t.Run(provider, func(t *testing.T) {
			...
			stream, err := model.New(provider, model.WithAPIKey("test-key"), model.WithBaseURL(ts.URL)).Stream(context.Background(), &model.Request{Prompt: "Hello"})
			if err != nil {
				t.Fatalf("Stream returned error: %v", err)
			}
			assertStreamReply(t, stream, "hel")
...
func TestStreamProvidersHonorCanceledContextBeforeRequest(t *testing.T) {
	for _, provider := range conformingStreamProviders(t) {
		...
		t.Run(provider, func(t *testing.T) {
			...
			cancel()
			stream, err := model.New(provider, model.WithAPIKey("test-key"), model.WithBaseURL(ts.URL)).Stream(ctx, &model.Request{Prompt: "Hello"})
			if err == nil {
				_ = stream.Close()
...
func TestConfiguredProviderStreamsSkipWithoutCredentials(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.provider, func(t *testing.T) {
			...
			opts := []model.Option{model.WithAPIKey(key)}
			if modelName := os.Getenv(tc.modelEnv); modelName != "" {
...
func conformingStreamProviders(t *testing.T) []string {
	t.Helper()
	providers := model.RegisteredProviders("stream")
...
func assertStreamReply(t *testing.T, stream model.Stream, want string) {
	t.Helper()
	chunk, err := stream.Recv()
	if err != nil {
