...
//
// It used to be compiled into the core registry package (its impl lived in
// registry/mdns_registry.go) and was the package-default discovery. It now
// links the multicast DNS stack (miekg/dns) only when needed: import this
// package — directly, or via go-micro.dev/v6/cmd/defaults — to register the
// "mdns" registry and use it as the default.
//
//	reg := mdns.NewRegistry()
//	// or, to make it the package default:
...
import (
	...
	utilmdns "go-micro.dev/v6/internal/util/mdns"
...
type mdnsRegistry struct {
	...
	// listener
	listener chan *utilmdns.ServiceEntry
	// the mdns domain
	domain string
...
func (m *mdnsRegistry) Register(service *registry.Service, opts ...registry.RegisterOption) error {
	...
	if !ok {
		...
		srv, err := utilmdns.NewServer(&utilmdns.Config{Zone: &utilmdns.DNSSDService{MDNSService: s}})
		if err != nil {
			return err
		}
	...
	for _, node := range service.Nodes {
		...
		srv, err := utilmdns.NewServer(&utilmdns.Config{Zone: s, LocalhostChecking: true})
		if err != nil {
			gerr = err
			continue
		}
...
func (m *mdnsRegistry) GetService(service string, opts ...registry.GetOption) ([]*registry.Service, error) {
	...
	entries := make(chan *utilmdns.ServiceEntry, 10)
	done := make(chan bool)

	p := utilmdns.DefaultParams(service)
	...
	// execute the query
	if err := utilmdns.Query(p); err != nil {
		return nil, err
	}
...
func (m *mdnsRegistry) ListServices(opts ...registry.ListOption) ([]*registry.Service, error) {
	...
	p := utilmdns.DefaultParams("_services")
	// set context with timeout
	var cancel context.CancelFunc
	p.Context, cancel = context.WithTimeout(context.Background(), m.opts.Timeout)
	defer cancel()
...
func (m *mdnsRegistry) Watch(opts ...registry.WatchOption) (registry.Watcher, error) {
	...
	md := &mdnsWatcher{
		id:       uuid.New().String(),
		wo:       wo,
		ch:       make(chan *utilmdns.ServiceEntry, 32),
	...
	go func() {
		...
		for {
			...
			go func() {
				send := func(w *mdnsWatcher, e *utilmdns.ServiceEntry) {
					select {
					case w.ch <- e:
					default:
					}
...
// NewRegistry returns a new multicast dns registry.
func NewRegistry(opts ...registry.Option) registry.Registry {
