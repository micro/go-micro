...
func NewRegistry(opts ...registry.Option) registry.Registry {
	...
	reg := &Registry{
		options:  options,
		records:  map[string]services{options.Domain: records},
...
func (m *Registry) Init(opts ...registry.Option) error {
	...
	srvs, ok := m.records[m.options.Domain]
	if !ok {
		srvs = make(services)
	}

	// loop through the services and if it doesn't yet exist, add it to the slice
	...
	m.records[m.options.Domain] = srvs
	return nil
}
...
func (m *Registry) Register(s *registry.Service, opts ...registry.RegisterOption) error {
	...
	if len(options.Domain) == 0 {
		options.Domain = m.options.Domain
	}
...
func (m *Registry) ListServices(opts ...registry.ListOption) ([]*registry.Service, error) {
	...
	for _, service := range services {
		for _, version := range service {
			result = append(result, recordToService(version))
		}
	}
	return result, nil
}
