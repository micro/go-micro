...
func (c *registryService) Sync(ctx context.Context, in *SyncRequest, opts ...client.CallOption) (Registry_SyncService, error) {
	...
	return &registryServiceSync{stream}, nil
}

type Registry_SyncService interface {
...
type registryServiceSync struct {
	stream client.Stream
}

func (x *registryServiceSync) Close() error {
	return x.stream.Close()
}

func (x *registryServiceSync) SendMsg(m interface{}) error {
	return x.stream.Send(m)
}

func (x *registryServiceSync) RecvMsg(m interface{}) error {
	return x.stream.Recv(m)
}

func (x *registryServiceSync) Recv() (*SyncResponse, error) {
	m := new(SyncResponse)
	err := x.stream.Recv(m)
	if err != nil {
		return nil, err
...
func (h *registryHandler) Sync(ctx context.Context, stream server.Stream) error {
	...
	return h.RegistryHandler.Sync(ctx, m, &registrySyncStream{stream})
}

type Registry_SyncStream interface {
	SendMsg(interface{}) error
	RecvMsg(interface{}) error
	Close() error
...
type registrySyncStream struct {
	stream server.Stream
}

func (x *registrySyncStream) Close() error {
	return x.stream.Close()
}

func (x *registrySyncStream) SendMsg(m interface{}) error {
	return x.stream.Send(m)
}

func (x *registrySyncStream) RecvMsg(m interface{}) error {
	return x.stream.Recv(m)
}
