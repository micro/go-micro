// Package file is a file-backed store (single-file bbolt per table).
//
// It used to be compiled into the core store package (its impl lived in
// store/file.go) and was the package-default store. It now links bbolt only
// when needed: import this package — directly, or via
// go-micro.dev/v6/cmd/defaults — to register the "file" store and make it
// the default (NewStore + DefaultStore).
//
//	store.RegisterDefault(file.NewStore) // or via cmd/defaults
package file
